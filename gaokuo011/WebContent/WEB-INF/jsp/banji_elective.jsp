...
<title>Insert title here</title>
	...
	function selectBanji(){
		...
		if(banjiId != "选择班级"){
			$.post(
		           "${pageContext.request.contextPath}/banji/findCoursesByBanjiId.action", 
		           {"banji_id":banjiId}, //data
		           function(data) { //callback
		           $(".useDel").remove();
		           for(var p in data){//遍历json数组时，p为索引，0,1.....
		        	   //$("#hadCourses").after("<a href='javascript:delCourse("+data[p].banji_id+","+data[p].course_id+")'>"+data[p].course_name+"&nbsp;&nbsp;&nbsp;<a/>");
		           		$("#hadCourses").after("<a class='useDel' href='javascript:delCourse("+data[p].course_id+")'>"+data[p].course_name+"&nbsp;&nbsp;&nbsp;<a/>");
		        	}
		           },
		           "json" //type
		       );
	...
	function delCourse(course_id){
		...
		var isModify = confirm("想好没？");
	       if (isModify) {
	    	   $.post(
		               "${pageContext.request.contextPath}/banji/deleteElective.action", 
		               {"banji_id":banji_id,
	...
	$(function() {
	       $("#addLine").click(function(){
	           ...
	           if(banji_id != "选择班级"){
		           $.post(
		               "${pageContext.request.contextPath}/banji/elective.action", //url
		               {"coursesId":coursesId,
		            	"banji_id":banji_id,
		               }, //data
		               function(data){
			...
			<div class="row">
				<div class="col-sm-2">
					<div class="list-group">
						<a href="${pageContext.request.contextPath}/course/find.action" class="list-group-item">班级列表</a>
						<a href="${pageContext.request.contextPath}/course/goadd.action" class="list-group-item">
							添加班级
						</a>
						<a href="${pageContext.request.contextPath}/course/goelective.action" class="list-group-item active">
							班级选课
						</a>
					</div>
				</div>
