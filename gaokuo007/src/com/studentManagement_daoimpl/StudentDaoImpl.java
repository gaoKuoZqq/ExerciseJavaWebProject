...
public class StudentDaoImpl implements IStudentDao{
	...
	public int addStudent(Student student) {
		...
		try {
			...
			String sql = "insert into student(name,age,gender,birthday,banji_id) values(?,?,?,?,?);";
			preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, student.getName());
			preparedStatement.setInt(2, student.getAge());
			preparedStatement.setString(3, student.getGender());
			preparedStatement.setDate(4, new java.sql.Date(student.getBirthday().getTime()));
			preparedStatement.setInt(5, student.getbanji().getBanjiId());
	...
	public int modifyStudent(Student student) {
		...
		try {
			connection = JdbcUtil.getConnection();
			String sql = "update student set name=?,age=?,gender=?,birthday=?,banji_id=? where id=?;";
			preparedStatement =  connection.prepareStatement(sql);
			preparedStatement.setString(1, student.getName());
			preparedStatement.setInt(2, student.getAge());
			preparedStatement.setString(3, student.getGender());
	...
	public List<Student> queryStudents(Student student, int pageIndex, int pageSize) {
		...
		try {
			...
			if (student.getbanji() != null && student.getbanji().getBanjiId() != null && student.getbanji().getBanjiId() != 0) {
				sql = sql + " and banji_id=?";
				list.add(String.valueOf(student.getbanji().getBanjiId()));
			}
			...
			while(resultSet.next()){
				...
				Student stu = new Student(id,name,age,gender,birthday,banji);
				studentList.add(stu);
			}
